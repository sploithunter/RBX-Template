...
-- Does the pet a slot points at actually still exist in the inventory? Stale slots
-- (saved state for pets that were traded/removed) would otherwise blindly spawn a
-- phantom. FAIL OPEN: only return false when the inventory mirror is present AND the
-- specific pet is absent — never when the structure looks unloaded, so a load-timing
-- glitch can't wrongly unequip a real pet.
local function slotHasBacking(player, parsed)
    ...
    if parsed.kind == "stack" then
        ...
        if not stacks then
            return true -- mirror not built yet
        end
        local stack = stacks:FindFirstChild(parsed.stackKey)
        local qty = (stack and stack:FindFirstChild("Quantity") and stack.Quantity.Value) or 0
        return qty > 0
    ...
    elseif parsed.kind == "legacy" then
        ...
        if special:FindFirstChild(parsed.raw) or petsFolder:FindFirstChild(parsed.raw) then
            return true
        end
        return false
    end
